...
11. **Ground every test stream in real-world research.** Never compile a stream from the user's description alone: before writing the units, research how this kind of feature actually works (how a stop-loss behaves on a real exchange, its states, what can go wrong) and start from that understanding. Research is a guide, never the source of truth — the user's idea is the North Star; where they disagree, the idea wins and the disagreement gets named. **Broadcast, never silent:** the disagreement lands in a strip at the TOP of the rendered page (a disagreement with the idea is a disagreement with every stream built from it — the reader sees it before trusting a unit), stays as a `Research note:` gotcha on the feature (the tester never re-grades the bar mid-run), and is repeated in the reply to Bobby — keep the map (note stays as documentation) or update the idea (map and stream change, note comes off). Never quietly rewrite the bar from research; never drop a disagreement silently — silent means Bobby never finds out. Novel or composite features with no existing model: break them into known parts (dynamic stop-loss = basic stop-loss + offset + how the offset moves), research the parts, recompose.
