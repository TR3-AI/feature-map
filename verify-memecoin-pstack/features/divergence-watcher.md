...
Existence: bot-simulated, and more so than the map implies. Among the providers the idea names, only Birdeye exposes an OHLCV candles API. Dexscreener's public API has no candles endpoint, and Helius offers RPC, DAS, and webhooks, not candles. None of the three returns OBV or RSI. So the bot sources the candles from whichever provider is pinned (PRE-BUILD) and computes OBV and RSI itself, then detects divergence candle by candle. That is exactly why the confirmed-versus-repainting distinction has to be built and verified deliberately, never trusted from an off-the-shelf indicator.
Deviations from standard: the map's "regular divergence only, confirmed on closed candles" logic matches standard TA practice. The clarification is the feed. The map reads as if a pinned provider supplies OHLC plus OBV plus RSI, but no named provider supplies the oscillators, and only Birdeye supplies candles at all, so both oscillators are the bot's own computation and the provider choice is PRE-BUILD. See the research note below.
...
- Research note: the map lists the chart feed as supplying "OHLC + OBV + RSI," but primary API docs show otherwise. Among the named providers, only Birdeye has an OHLCV candles endpoint; Dexscreener's public API has no candles endpoint and Helius has none (RPC, DAS, and webhooks only), and none of them return OBV or RSI. Tester action: pin the candle provider (still PRE-BUILD) and verify the bot's own OBV and RSI math against a known reference series first (a fixed candle set with hand-checked oscillator values), then test divergence on top. Do not expect any provider to send the oscillators.
