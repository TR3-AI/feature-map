...
- `provider-down` makes the candidate wait on a rate limit or outage. It never waves the candidate through with a fabricated number.
...
- `verdict-log` records every verdict, whether pass or reject, with the %, the trend, and the exact rule that fired.
...
1. **Trigger:** a candidate reaches this gate having already passed (or bypassed) Virality Gate and passed Narrative Gate. This is the last checkpoint before Clean.
...
3. **Surface:** the %, the exclusion detail, the trend label with its underlying readings, and the fired rule, all shown together on the candidate view. This becomes the candidate record's third stamp and its Clean/Rejected-at-gate-3 close.
4. **Breaks:** provider disagreement (different chain-analytics tools cluster wallets differently, so pin one provider and method, never average across tools) · an outage waved through as if a number exists · the stealth-bundling blind spot, where an atomic create-plus-buy reads as many unlinked holders to any provider, which is a provider limitation, not a pull bug · a single reading labeled as a real trend instead of unknown · a falling % read as unconditionally safe when it can also mean bundlers dumping into demand rather than diluting away · the ceiling treated as advisory ("close enough") · the direction check skipped in favor of the raw % alone, which is the single most dangerous failure, since a rising share is exactly what this rule exists to catch · a boundary off-by-one (exactly 15% wrongly rejected) · a verdict logged missing the %, trend, or fired rule.
...
- The pull comparison is against the *same* counting method. A different tool's number is expected to disagree, so use the pinned provider's own dashboard, not a competitor's.
