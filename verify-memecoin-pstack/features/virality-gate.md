...
- `verdict-log` records every verdict, whether pass, reject, or bypass, with the band, multiplier, and baseline behind it.
...
3. **Surface:** one verdict, whether pass, reject, or bypass, shown with its band, multiplier, and baseline together on the candidate view, becoming the candidate record's first stamp.
...
6. **verdict-log.** Check the log entries produced by every case above.
   Success: every verdict, whether pass, reject, or bypass, carries its band, multiplier, and baseline (where applicable) together, never a bare pass/reject.
...
- The 30-day baseline comes from the X API user-timeline endpoint (`GET /2/users/:id/tweets` with `public_metrics`), not the recent-search endpoint, which is capped at 7 days on every tier (full-archive search reaches back to 2006 but is Enterprise or pay-per-use). The timeline endpoint returns at most the 3,200 most-recent posts, so a high-volume account can exhaust that cap before 30 days are covered and leave the baseline built on a short window. Tester action: feed a high-post-count account and confirm the gate either falls back to full-archive search or fails closed with a visible incomplete-history reason, and never scores a silent partial window as a full 30-day baseline. The exact fallback, full-archive versus fail-closed, is PRE-BUILD.
