...
- The positions view in the Trader Console.

## How it works in practice

The mechanical chain the test stream walks:

1. **Trigger:** any position event from Position & Exit Ladder, entry, fill, cancel, close, or a price tick.
2. **Mechanism:** the dashboard streams unrealized P&L, entry-vs-current price, and order/stop state off Position & Exit Ladder's live feed. No manual refresh; closed positions drop to history; the moon bag sits in its own manual-only section.
...
Existence: live position and P&L dashboards are a standard, off-the-shelf UI pattern across trading platforms. This exists directly in the requested format; nothing needs bot-simulation beyond wiring it to real position events from Position & Exit Ladder.
Deviations from standard: none. Research reinforced the spec; the file's own gotcha about visibly flagging a stale or disconnected feed, rather than silently sitting on last-known numbers, matches how real live dashboards distinguish themselves from unreliable ones.
