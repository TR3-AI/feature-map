...
- [Callout Listener & Normalizer](./callout-listener.md): stream to four-field candidate, malformed rejected.
...
- [Virality Gate](./virality-gate.md): 30-day baseline and age-band multiplier (3x/10x/50x), no-tweet bypass.
- [Narrative Gate](./narrative-gate.md): Grok 4.6 five fields and dual score threshold (≥6 with tweet, >8 without).
- [Bundler Gate](./bundler-gate.md): creation-block % share and trend, ≤10–15% ceiling, increasing rejected harder.
...
- [Sell-Into-Volume Filter](./sell-into-volume.md): exits only into green candles and volume.
- [Fill Reporting](./fill-reporting.md): one event per fill, acknowledged by both consumers.

Position & Exit Ladder (one shared Position Record, sizing and the exit ladder):
