...
- `prefill` attaches the Fractional Kelly Sizer's size (Position & Exit Ladder).
- `once` fires exactly one alert per candidate.

## How to get to it (user POV)

- The alert card in the Trader Console.
...
3. **prefill.** Trigger the test divergence and check the size shown on the alert against the Fractional Kelly Sizer's calculation in Position & Exit Ladder.
