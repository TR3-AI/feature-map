...
4. **Breaks:** the ghost quote, a frozen feed that delivers no events at all, so a purely per-candle guard has nothing to count and never fires (the structural blind spot already flagged in this file's Research note) · a guard fires but the watch keeps tracking (the stop must be final).
