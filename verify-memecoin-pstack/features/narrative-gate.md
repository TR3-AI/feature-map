...
Existence: prompt → LLM call → schema-validated JSON is standard, off-the-shelf tooling today. Structured outputs / JSON mode plus an app-side validator exist directly, nothing here needs bot-simulation beyond forcing the two failure shapes for the test. Combining scored inputs into one number and gating it against a calibrated, context-shifted bar with a logged verdict is a standard trading risk-gate pattern. The virality (attention) score depends on Grok's live X access. As of 2026-09-07 that path is the server-side `x_search` tool on the xAI Responses API (`/v1/responses`), with the five-field extraction pinned by the Responses API's structured-output config `text.format` (the `xai-sdk` / OpenAI-2.x `text_format`, parsed into `output_parsed`); `response_format` is the Chat Completions equivalent and does not apply on the Responses path. xAI retired the legacy Live Search `search_parameters` path on 2026-01-12, and it now returns 410 Gone, so a build wired to the old path fails at the source rather than at the schema check. The exact call path stays PRE-BUILD until the repo exists.
