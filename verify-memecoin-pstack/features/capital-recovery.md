...
2. **Mechanism:** the bot sells the initial stake on the live pool (a real AMM trade where thin depth can slippage-reject it, requiring a retry), confirms the sell landed against the exchange's own record, and only then cancels the stop.
...
Existence: this is bot-simulated. No exchange offers a native "withdraw initial at 2x, cancel stop" order type; the bot watches price and sequences sell → confirm → cancel against the exchange's own order state.
