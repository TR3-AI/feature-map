...
4. The Trader Console shows the active list so Bobby can see exactly who is being watched.
Behaviour:
...
2. An append-only stamp log on the record: each gate — Virality, Narrative, Bundler — writes exactly one stamp bearing its name, its verdict, and (on reject) the reason, never overwriting a prior stamp.
...
- A no-tweet candidate still receives a Virality Gate stamp — a documented bypass verdict, not a missing one — so a Clean record always shows exactly three stamps, never two.
...
3. Each gate runs in fixed order — Virality, then Narrative, then Bundler — reading the record, doing its own work, and writing exactly one stamp before handing the record on.
...
2. It checks each stamp log in order — three passing stamps for the clean candidate, exactly two stamps (pass, then a named reject reason) for the rejected one — and confirms the rejected candidate never shows a third gate's stamp, then forces a duplicate evaluation of one gate to confirm the log still shows only one stamp for it.
Success: The candidate view shows the clean candidate closed Clean with all three gate stamps present and correctly ordered, and the rejected candidate closed Rejected-at-gate-2 with exactly two stamps and a visible reason on the second — with a forced duplicate gate evaluation still producing only one stamp for that gate, all inside the recording.
...
5. A verdict log recording every outcome — pass, reject, or bypass — together with the band, multiplier, and baseline that produced it, feeding the candidate record's first stamp.
Behaviour:
...
3. An app-side schema validator on top of that structured response — required fields present, veracity and virality present as two distinct in-range numbers — with a bounded retry before a hard, fail-closed block; a dead API and a schema-valid-but-wrong-shape response are both treated as the same failure.
4. Veracity and virality kept as separate score components through extraction and combination — never merged, never renamed into each other.
...
Lifecycle:
1. TRIGGER: a candidate arrives from Virality Gate (passed or bypassed) or directly from Callout Intake with no tweet.
...
Success: All four scored candidates verdict correctly against the dual rule with the branch and threshold visible, the boundary values (6.0/5.9, 8.0/8.01) land on the correct side of their thresholds, the empty-but-present tweet field is treated as the no-tweet branch, both extracted scores stay visibly separate throughout, and both the dead-API and the schema-violating-200 cases show the candidate blocked with a specific error — all in the recording.
...
Success: The three known candidates verdict pass, reject, reject with the %, trend, and fired rule all visible; the exact 15% boundary passes rather than rejects; the single-snapshot token shows unknown rather than any real trend; and the blocked provider produces a visible wait followed by a fresh (not stale, not fabricated) number on restore — all in the recording.
...
2. The pre-filled size from the Fractional Kelly Sizer's calculation, attached to the payload — the Kelly Sizer now lives in Position & Exit Ladder, not the old "Position manager" department.
...
4. END: Fill reported to Position & Exit Ladder + Trade Journal; failure reported with the chain error.
Verification:
...
3. Position & Exit Ladder's ladder state advances on both.
...
2. A running realized/unrealized split and live quantity, updated one fill at a time — the 2x withdrawal, each clip, a stop-loss — off the frozen fill-event schema.
...
3. Fills post one at a time — the 2x withdrawal, the moon-bag flag, each divergence clip, or a stop-loss — the record's quantity and realized/unrealized split advancing after every one, checked periodically against the venue's own position.
...
From: Position & Exit Ladder
Feature:
1. After 2x, **20% of the remaining position** is flagged as the moon bag in the shared Position Record.
2. The flag is exclusion: the clip ladder and every automatic exit skip the bag.
3. The bag's home is Trader Console's Positions & Moon Bag View — view it, sell it manually; no automation touches it.
Behaviour:
